#!/usr/bin/env python3
"""Quiz 16: Food & Cuisine by Country — Image Generator (ModelsLab API)
Usage: python3 generate_images.py
Requires: MODELSLAB_API_KEY in .env
NOTE: wyr_scene rows need NO images — they are drawn procedurally by make_quiz_video.py
using Twemoji. This script only generates title_card and question_scene images.
"""
...
NEG = "blurry, low quality, distorted, ugly, watermark, text, letters, words, writing, typography, cartoon, anime, people, faces, persons, logos"

def load_images():
    images = []
    with open(CSV_FILE, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row["asset_type"] not in ("title_card", "question_scene", "outro"):
                continue  # skip wyr_scene — no image needed
            fn = row.get("file_name", "").strip()
            prompt = row.get("image_prompt", "").strip()
            if not fn or not prompt:
                continue
            images.append((fn, prompt + ", cinematic wide shot, ultra photorealistic, 8k, professional photography, dramatic lighting"))
    return images
...
def main():
    ...
    print(f"(wyr_scene rows skip image generation — drawn live via Twemoji)\n")
    errors = []
